export const appAction = {
  TEST_PROMISE: 'TEST_PROMISE',
  TEST_PROMISE_SUCCESS: 'TEST_PROMISE_SUCCESS',
  TEST_PROMISE_FAILURE: 'TEST_PROMISE_FAILURE',
};

export const authAction = {
  IS_LOGIN_COMPONENT: 'IS_LOGIN_COMPONENT',
  IS_SIGNUP_COMPONENT: 'IS_SIGNUP_COMPONENT',

  ON_LOGIN: 'ON_LOGIN',
  ON_LOGIN_SUCCESS: 'ON_LOGIN_SUCCESS',

  ON_SIGNUP: 'ON_SIGNUP',
  ON_SIGNUP_SUCCESS: 'ON_SIGNUP_SUCCESS',

  LOAD_AUTH: 'LOAD_AUTH',
  LOAD_AUTH_SUCCESS: 'LOAD_AUTH_SUCCESS',

  ON_LOGOUT: 'ON_LOGOUT',
  ON_LOGOUT_SUCCESS: 'ON_LOGOUT_SUCCESS',
};

export const eventAction = {
  LOAD_EVENTS_LIST: 'LOAD_EVENTS_LIST',
  LOAD_EVENTS_LIST_SUCCESS: 'LOAD_EVENTS_LIST_SUCCESS',

  LOAD_EVENT_DETAIL: 'LOAD_EVENTS_DETAIL',
  LOAD_EVENT_DETAIL_SUCCESS: 'LOAD_EVENTS_DETAIL_SUCCESS',
};

export const qrAction = {
  CHECK_QR: 'CHECK_QR',
  CHECK_QR_SUCCESS: 'CHECK_QR_SUCCESS',
};

export const guestAction = {
  LOAD_UNACCEPTED_GUESTS_LIST: 'LOAD_UNACCEPTED_GUESTS_LIST',
  LOAD_UNACCEPTED_GUESTS_LIST_SUCCESS: 'LOAD_UNACCEPTED_GUESTS_LIST_SUCCESS',
  LOAD_ACCEPTED_GUESTS_LIST: 'LOAD_ACCEPTED_GUESTS_LIST',
  LOAD_ACCEPTED_GUESTS_LIST_SUCCESS: 'LOAD_ACCEPTED_GUESTS_LIST_SUCCESS',
  LOAD_CHECKEDIN_GUESTS_LIST: 'LOAD_CHECKEDIN_GUESTS_LIST',
  LOAD_CHECKEDIN_GUESTS_LIST_SUCCESS: 'LOAD_CHECKEDIN_GUESTS_LIST_SUCCESS',

  ACCEPT_TICKET: 'ACCEPT_TICKET',
  ACCEPT_TICKET_SUCCESS: 'ACCEPT_TICKET_SUCCESS',
};
